...
import com.cpic.springbatch.decider.MyDecider;
...
public class DeciderDemo {
    ...
    public Job deciderDemoJob() {
        return jobBuilderFactory.get("deciderDemoJob")
                ...
                .next(myDecider())
                .from(myDecider()).on("even").to(deciderDemoStep2())//根据决策器返回结果确定后续执行step
                .from(myDecider()).on("odd").to(deciderDemoStep3())
                .from(deciderDemoStep3()).on("*").to(myDecider())//不管step结果如何都回到决策器
                .end().build();
    }
}